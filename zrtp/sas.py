...
def b32(sas):
  ...
  for i in range(0, 4):
    n = (u32>>shift) & 31
    result += 'ybndrfg8ejkmcpqxot1uwisza345h769'[n]
    shift -= 5
...
def get(name):
  sas = None
  for (sname, constructor) in SASS:
    if sname == name: return constructor
  if sas is None: raise UnsupportedSASType(name)

if __name__ == '__main__':
  def sas(name):
    return get(name)

  assert(sas(b'B256')(b'\x53\x3c\x50\x00') == 'dwelling:crossover')
  assert(sas(b'B256')(b'\xed\xfc\x00\x00') == 'tunnel:Wilmington')

  assert(sas(b'B32 ')(b'\x2a\xf2\xb0\x00') == 'fm3m')
  assert(sas(b'B32 ')(b'\xde\xfa\x40\x00') == '557r')
